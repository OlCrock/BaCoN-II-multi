...
def generate_noise(k, P, 
  ...
  if noise_model == 'default':
      ...
      beta = f / bias
      # For log-spaced k, each bin has a different width
      delta_k_log = np.diff(k)
      delta_k_log = np.append(delta_k_log, delta_k_log[-1])  # pad last bin to match length

     # Use monopole (P0) to convert to linear power spectrum 
     # Note this is only valid for Kaiser! Perhaps best if we have a fiducial (Planck) linear spectrum hardcoded or fed in 
      monopole_factor = (bias**2 * (1.0 + 2.0/3.0*beta + 1.0/5.0*beta**2))**(-1)
      P_lin_use = monopole_factor*P[:, 0:1]  # shape (nk, 1), monopole only
      k_hMpc = k 
      Nk = 4*np.pi*(k_hMpc**2)[:, None] * delta_k_log[:, None] * (veff) / (2.0 * np.pi)**3
      ...
      C22 = (2.0 / Nk) * (
          ...
          + (2/ng) * (5.0 + (110.0/21.0)*beta + (15.0/7.0)*beta**2) * bl2P
          + 5.0/ng**2
      ) 
      ...
      # --- Invert 3x3 covariance matrix at each k, take diagonal of inverse, consistent with mathematica notebook ---
      nk_bins = P.shape[0]
      sigma_noise = np.zeros_like(P)
...
class DataGenerator(tf.compat.v2.keras.utils.Sequence): # need to add new variable to 'params' further down
    def __init__(self, list_IDs, labels, labels_dict, batch_size=32, 
                ...
                noise_model='default',
                kaiser_bias=1.0, kaiser_ng=None, kaiser_veff=0.28,
                ):
              ...
              for i_noise in range(self.n_noisy_samples):
                if self.add_noise:
                  ...
                  if self.add_cosvar:
                    noise_kw = {}
                    if self.noise_model == 'linear_kaiser':
                        noise_kw = {'bias': self.kaiser_bias, 'ng': self.kaiser_ng, 'veff': self.kaiser_veff}
                    noise_cosVar = np.random.normal(loc=0, scale=generate_noise(k,self.norm_data[: , self.z_bins],sys_scaled=self.sys_scaled,sys_factor=self.sys_factor,sys_max=self.sys_max, add_cosvar=True, add_sys=False, add_shot=False,sigma_sys=self.sigma_sys, noise_model=self.noise_model, **noise_kw))
                  ...
                  if self.add_shot:
                    noise_kw_shot = {}
                    if self.noise_model == 'linear_kaiser':
                        noise_kw_shot = {'bias': self.kaiser_bias, 'ng': self.kaiser_ng, 'veff': self.kaiser_veff}
                    noise_shot = np.random.normal(loc=0, scale=generate_noise(k,self.norm_data[: , self.z_bins],sys_scaled=self.sys_scaled,sys_factor=self.sys_factor,sys_max=self.sys_max, add_cosvar=False, add_sys=False, add_shot=True,sigma_sys=self.sigma_sys, noise_model=self.noise_model, **noise_kw_shot))
...
def create_test_generator(FLAGS):
    ...
    params_test = {'dim': (FLAGS.im_depth, FLAGS.im_width),
          ...
          'fname':FLAGS.fname,
          'noise_model':FLAGS.noise_model,
          'kaiser_beta':FLAGS.kaiser_beta,
          'kaiser_f':FLAGS.kaiser_f,
          'kaiser_bias':FLAGS.kaiser_bias,
          'kaiser_ng':FLAGS.kaiser_ng,
          #'c_0':FLAGS.c_0 ,
          #'c_1':FLAGS.c_1 ,  
