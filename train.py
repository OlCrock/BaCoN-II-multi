...
def compute_kaiser_f(model_name, z=0.1, Omega_m=0.315, h=0.674, w0=-1.0, wa=0.0, omegarc=None):
    """
    Compute linear growth rate f from growth function using MGrowth.
    
    Parameters:
    -----------
    model_name : str
        Cosmological model: 'lcdm', 'wcdm', 'dgp' (nDGP), or 'fR'
    z : float
        Redshift at which to compute growth rate (default 0.1)
    Omega_m : float
        Matter density parameter
    h : float
        Hubble constant in units of 100 km/s/Mpc
    w0 : float
        Dark energy equation of state at z=0 (for wCDM)
    wa : float
        Variation of w with scale factor (for wCDM)
    omegarc : float
        Omega_rc parameter for nDGP
    ...
    if model_name_lower == 'lcdm':
        cosmo = mg.LCDM(bg_params)
        D, f = cosmo.growth_parameters()
    elif model_name_lower == 'wcdm':
        cosmo = mg.wCDM(bg_params)
        D, f = cosmo.growth_parameters()
    elif model_name_lower in ['dgp', 'ndgp']:
        if omegarc is None:
            raise ValueError("omegarc parameter required for nDGP model")
        cosmo = mg.nDGP(bg_params)
        D, f = cosmo.growth_parameters(omegarc)
    elif model_name_lower == 'fr':
        raise ValueError("f(R) requires additional parameters. Set kaiser_f manually for f(R).")
    else:
...
def main():
    ...
    parser.add_argument("--im_depth", default=1000, type=int, required=False)
    ...
    parser.add_argument("--noise_model", default='default', type=str, required=False)
    parser.add_argument("--kaiser_bias", default=1.0, type=float, required=False)
    parser.add_argument("--kaiser_ng", default=None, type=float, required=False)
    parser.add_argument("--kaiser_veff", default=0.28, type=float, required=False)
