...
def main():
   ...
   noise_model='default'  # Noise model type: 'default' or 'linear_kaiser'
   
   # Kaiser linear model parameters (only needed for linear_kaiser noise model)
   kaiser_bias=1.0   # Galaxy bias for Kaiser model
   kaiser_ng=None    # Galaxy number density for Kaiser model
   kaiser_veff=0.28  # Effective survey volume in Gpc³/h³
   ...
   proc = subprocess.Popen(["python3", "train.py", "--test_mode" , test_mode, "--seed", seed, \
                     ...
                     "--kaiser_bias", str(kaiser_bias), \
                     "--kaiser_ng", str(kaiser_ng), \
                     "--kaiser_veff", str(kaiser_veff), \
