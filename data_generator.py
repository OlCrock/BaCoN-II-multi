...
# Planck fiducial linear growth rate f (hardcoded for covariance computation)
# Derived from Planck 2018 LCDM cosmology at z=0.1
PLANCK_GROWTH_RATE = 0.5854384289380487605
...
def generate_noise(k, P, 
                   ...
                   delta_k = 0.008,  sigma_sys=15, quadrature=True,
                   noise_model='default',
                   bias=1.0, ng=None, veff=0.28, P_lin=None):
  """
  Generate noise model for power spectrum.
  
  Parameters:
  -----------
  noise_model : str
      Type of noise model to use. Options: 'default', 'linear_kaiser'
      - 'default': Standard noise model
      - 'linear_kaiser': Linear Kaiser noise model for multipoles
  bias : float
      Galaxy bias parameter (only for linear_kaiser), default 1.0
  ng : float
      Galaxy number density (only for linear_kaiser)
  veff : float
      Effective survey volume in Gpc³/h³ (default 0.28)
  P_lin : array
      Linear power spectrum at each k (only for linear_kaiser).
      Should be loaded from normalisation file.
  
  Note:
      For the linear_kaiser model, the growth rate f used in the covariance
      is hardcoded as the Planck fiducial value (PLANCK_GROWTH_RATE).
      This is a survey-level property and does not vary per instance.
  """
  ...
  if noise_model == 'default':
  ...
  elif noise_model == 'linear_kaiser':
      # Linear Kaiser noise model for multipoles
      # Uses hardcoded Planck fiducial growth rate for covariance

      if ng is None:
          raise ValueError("linear_kaiser model requires ng parameter")
      if P_lin is None:
          raise ValueError("linear_kaiser model requires P_lin parameter (linear power spectrum from normalisation)")

      # Compute beta from Planck fiducial growth rate and bias
      f = PLANCK_GROWTH_RATE
      beta = f / bias

      # Use the full linear power spectrum passed in
      P_lin_use = np.atleast_1d(P_lin).copy()
      P_lin_use = P_lin_use[:, None]  # reshape to (nk, 1)
      
      # Use passed-in delta_k and veff parameters
      k_hMpc = k
      # Nk formula matching plots notebook exactly: 4πk²Δk × Veff / (2π)³
      # veff input is in Gpc³/h³, convert to (Mpc/h)³ by multiplying by 1e9
      Nk = 4*np.pi*(k_hMpc**2)[:, None] * delta_k * (veff * 1e9) / (2.0 * np.pi)**3
      bl2P = bias**2 * P_lin_use  # shorthand: b²P

      # --- Diagonal terms ---
      C00 = (2.0 / Nk) * (
          (1.0 + (4.0/3.0)*beta + (6.0/5.0)*beta**2 + (4.0/7.0)*beta**3 + (1.0/9.0)*beta**4) * bl2P**2
          + (2.0/ng) * (1.0 + (2.0/3.0)*beta + (1.0/5.0)*beta**2) * bl2P
          + (1.0/ng**2)
      )

      C22 = (2.0 / Nk) * (
          (5.0 + (220.0/21.0)*beta + (90.0/7.0)*beta**2 + (1700.0/231.0)*beta**3 + (2075.0/1287.0)*beta**4) * bl2P**2
          + (2.0/ng) * (5.0 + (110.0/21.0)*beta + (15.0/7.0)*beta**2) * bl2P
          + 5.0/ng**2
      )

      C44 = (2.0 / Nk) * (
          (9.0 + (1404.0/77.0)*beta + (104166.0/5005.0)*beta**2 + (11772.0/1001.0)*beta**3 + (6399.0/2431.0)*beta**4) * bl2P**2
          + (1.0/ng) * (18.0 + (1404.0/77.0)*beta + (34722.0/5005.0)*beta**2) * bl2P
          + 9.0/ng**2
      )

      # --- Off-diagonal terms ---
      C02 = (2.0 / Nk) * (
          ((8.0/3.0)*beta + (24.0/7.0)*beta**2 + (40.0/21.0)*beta**3 + (40.0/99.0)*beta**4) * bl2P**2
          + (2.0/ng) * ((4.0/3.0)*beta + (4.0/7.0)*beta**2) * bl2P
      )

      C04 = (2.0 / Nk) * (
          ((48.0/35.0)*beta**2 + (96.0/77.0)*beta**3 + (48.0/143.0)*beta**4) * bl2P**2
          + (1.0/ng) * ((16.0/35.0)*beta**2) * bl2P
      )

      C24 = (2.0 / Nk) * (
          ((48.0/7.0)*beta + (816.0/77.0)*beta**2 + (6960.0/1001.0)*beta**3 + (240.0/143.0)*beta**4) * bl2P**2
          + (1.0/ng) * ((48.0/7.0)*beta + (272.0/77.0)*beta**2) * bl2P
      )

      # --- Invert 3x3 covariance matrix at each k using analytic formula ---
      nk_bins = P.shape[0]
      sigma_noise = np.zeros((nk_bins, 3))

      for i in range(nk_bins):
          c00 = C00[i, 0]; c22 = C22[i, 0]; c44 = C44[i, 0]
          c02 = C02[i, 0]; c04 = C04[i, 0]; c24 = C24[i, 0]

          # Analytic determinant (matching plots notebook)
          det = (c00*(c22*c44 - c24**2) 
              - c02*(c02*c44 - c24*c04) 
              + c04*(c02*c24 - c22*c04))

          # Diagonal elements of inverse (from Mathematica)
          c00i = (c22*c44 - c24**2) / det
          c22i = (c00*c44 - c04**2) / det
          c44i = (c00*c22 - c02**2) / det

          sigma_noise[i, 0] = 1.0 / np.sqrt(c00i)
          sigma_noise[i, 1] = 1.0 / np.sqrt(c22i)
          sigma_noise[i, 2] = 1.0 / np.sqrt(c44i)

      if add_shot:
          # Shot noise already included in Kaiser covariance
          pass
      if add_sys:
          if sys_scaled:
              if quadrature:
      ...
      raise ValueError(f"Unknown noise_model: {noise_model}. Choose 'default' or 'linear_kaiser'")
...
class DataGenerator(tf.compat.v2.keras.utils.Sequence): # need to add new variable to 'params' further down
    def __init__(self, list_IDs, labels, labels_dict, batch_size=32, 
                ...
                noise_model='default',
                kaiser_bias=1.0, kaiser_ng=None, kaiser_veff=0.28, kaiser_delta_k=0.008,
                ):
        ...
        self.kaiser_bias=kaiser_bias
        self.kaiser_ng=kaiser_ng
        self.kaiser_veff=kaiser_veff
        self.kaiser_delta_k=kaiser_delta_k
    ...
    def __data_generation(self, list_IDs_temp, list_IDs_temp_dict):
        ...
        for f_ind in range(len(fname_list)):            
            # Pick corresponding file from each folder 
              ...
              # Strip fR0 and omegarc columns if present (linear_kaiser data format)
              if self.noise_model == 'linear_kaiser':
                  P_original = P_original[:, :3]      # keep only P0, P2, P4
              ...
              for i_noise in range(self.n_noisy_samples):
                if self.add_noise:
                  ...
                  if self.add_cosvar:
                    noise_kw = {}
                    if self.noise_model == 'linear_kaiser':
                        # Always use the final column as P_lin (linear/reference power spectrum)
                        p_lin_ref = self.norm_data[:, -1]
                        noise_kw = {'bias': self.kaiser_bias, 'ng': self.kaiser_ng, 'veff': self.kaiser_veff, 'delta_k': self.kaiser_delta_k, 'P_lin': p_lin_ref}
                    noise_full = generate_noise(k,self.norm_data[: , self.z_bins],sys_scaled=self.sys_scaled,sys_factor=self.sys_factor,sys_max=self.sys_max, add_cosvar=True, add_sys=False, add_shot=False,sigma_sys=self.sigma_sys, noise_model=self.noise_model, **noise_kw)
                    noise_cosVar = np.random.normal(loc=0, scale=noise_full[:, :self.norm_data[:, self.z_bins].shape[1]])
                  ...
                  if self.add_shot:
                    ...
                    if self.noise_model == 'linear_kaiser':
                        # Always use the final column as P_lin (linear/reference power spectrum)
                        p_lin_ref = self.norm_data[:, -1]
                        noise_kw_shot = {'bias': self.kaiser_bias, 'ng': self.kaiser_ng, 'veff': self.kaiser_veff, 'delta_k': self.kaiser_delta_k, 'P_lin': p_lin_ref}
                    noise_full = generate_noise(k,self.norm_data[: , self.z_bins],sys_scaled=self.sys_scaled,sys_factor=self.sys_factor,sys_max=self.sys_max, add_cosvar=False, add_sys=False, add_shot=True,sigma_sys=self.sigma_sys, noise_model=self.noise_model, **noise_kw_shot)
                    noise_shot = np.random.normal(loc=0, scale=noise_full[:, :self.norm_data[:, self.z_bins].shape[1]])
...
def create_generators(FLAGS):
    ...
    try:
        ...
        print(' ####  FLAGS.noise_model not found! #### \n Probably loading an older model. Using noise_model=default')
        noise_model='default'
    try:
        kaiser_bias=FLAGS.kaiser_bias
    except AttributeError:
        print(' ####  FLAGS.kaiser_bias not found! #### \n Using kaiser_bias=1.0')
        kaiser_bias=1.0
    try:
        kaiser_ng=FLAGS.kaiser_ng
    except AttributeError:
        print(' ####  FLAGS.kaiser_ng not found! #### \n Using kaiser_ng=None')
        kaiser_ng=None
    try:
        kaiser_veff=FLAGS.kaiser_veff
    except AttributeError:
        print(' ####  FLAGS.kaiser_veff not found! #### \n Using kaiser_veff=0.28')
        kaiser_veff=0.28
    try:
        kaiser_delta_k=FLAGS.kaiser_delta_k
    except AttributeError:
        print(' ####  FLAGS.kaiser_delta_k not found! #### \n Using kaiser_delta_k=0.008')
    ...
    params = {'dim': (FLAGS.im_depth, FLAGS.im_width),
          ...
          'kaiser_bias':kaiser_bias,
          'kaiser_ng':kaiser_ng,
          'kaiser_veff':kaiser_veff,
          'kaiser_delta_k':kaiser_delta_k,
          #'c_0':FLAGS.c_0 ,
          #'c_1':FLAGS.c_1 ,    
