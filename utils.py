...
def get_all_indexes(FLAGS, Test=False):
    ...
    for l in all_labels:
        ...
        files = [name for name in os.listdir(dir_name) if (os.path.isfile(os.path.join(dir_name, name)) and 'DS_Store' not in name)]
        n_samples = len(files)
        print('%s - %s training examples' %(l,n_samples))
        n_s.append(n_samples)
        try:
            idxs = [int(str.split(name, sep='.')[0]) for name in files]
        except ValueError:
            raise ValueError('Non-integer file name encountered in %s. Expected files like 0.txt, 1.txt, ...' % dir_name)
        index_sets.append(set(idxs))

    if len(index_sets) == 0:
        raise ValueError('No class folders found in %s' % data_dir)

    common_indexes = set.intersection(*index_sets)
    if len(common_indexes) == 0:
        raise ValueError('No common file indexes across labels. Check dataset consistency.')

    all_index = np.array(sorted(common_indexes))
    n_samples = all_index.shape[0]
    print('\nN. of data files (common across labels): %s' %all_index.shape)
